...
class PrefixTuning(GPT2PreTrainedModel):
    ...
    def __init__(self, config, model_gpt2, optim_prefix=False, preseqlen=5, use_infix=False, deep_param=False):
        ...
        if hasattr(config, 'init_xavier'):
            self.init_xavier = (config.init_xavier == 'yes')
        else:
            self.init_xavier = False

        if hasattr(config, 'init_kaiming'):
            self.init_kaiming = (config.init_kaiming == 'yes')
        else:
        ...
        def init_weights(m):
            if isinstance(m, nn.Linear):
                if self.init_random:
                    print('random initialization of the prefix')
                    if m.weight.shape == torch.Size([self.mid_dim, config.n_embd]):
                        m.weight = nn.Parameter(torch.randn(self.mid_dim, config.n_embd))
                    else:
                        m.weight = nn.Parameter(torch.randn(config.n_layer * 2 * config.n_embd, self.mid_dim))
                elif self.init_xavier:
                    print('xavier initialization of the prefix')
                    nn.init.xavier_normal_(m.weight)
                elif self.init_kaiming:
                    print('kaiming initialization of the prefix')
        ...
        if True:
            ...
            # DIFFERENT PARAMETRIZATION:
                ...
                self.control_trans = nn.Sequential(
                    ...
                    nn.Tanh(),
                    nn.Linear(self.mid_dim, config.n_layer * 2 * config.n_embd))
                self.control_trans.apply(init_weights)
                if self.use_infix:
