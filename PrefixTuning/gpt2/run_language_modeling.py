...
class ModelArguments:
    ...
    init_xavier: Optional[str] = field(
        default='no',
        metadata={
            "help": "whether to init a xavier embedding, or use GPT2 embedding for the prefix tuning model. "
        },
    )

    init_kaiming: Optional[str] = field(
        default='no',
        metadata={
            "help": "whether to init a kaiming embedding, or use GPT2 embedding for the prefix tuning model. "
        },
    )
...
class DataTrainingArguments:
    ...
    finetuned_model_path: Optional[str] = field(
        default="/content/drive/MyDrive/Colab_Notebooks/PrefixTuning-cleaned/contrast_LM/transformers/examples/full/full/webnlgfinetune_n_20_act_cat_b=6-e"
...
def main():
    ...
    if model_args.tuning_mode == 'prefixtune': # prefixtune
        ...
        if model_args.prefixModel_name_or_path is not None:
            ...
            config_prefix.init_xavier = model_args.init_xavier
            config_prefix.init_kaiming = model_args.init_kaiming
    ...
    if 'lowdata' in training_args.output_dir:
        ...
        data_args.eval_data_file = "/content/drive/MyDrive/Colab_Notebooks/PrefixTuning-cleaned/data/e2e_data/src1_valid.txt"
        eval_dataset = (
            get_dataset(data_args, tokenizer=tokenizer, evaluate=True, cache_dir=model_args.cache_dir,
                        training_args=training_args, finetune_mode=(model_args.tuning_mode == 'finetune'))
            if training_args.do_eval
